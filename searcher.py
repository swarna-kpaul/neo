...
class neo():
    ...
    def actplan(self):
        ...
        if self.explore:
            ...
            self.explore = False
            model = llm_gpt4_turbo
            ACPtrace_text = "\n".join([ "    actionplan: "+ str(i["actionplan"]) for i in ACPtrace])
            ACPtrace_text = """Following are the historical action plans provided by you. The new action plan should be different from the old ones.
                     Action plan history:
                     """+ACPtrace_text
        else:
            ...
            ACPtrace_text = """Following are the historical action plans provided by you and the corresponding responses generated by the environment on executing the action plan. The new action plan should be different from the old ones.
                     Action plan history:
                     """+ACPtrace_text

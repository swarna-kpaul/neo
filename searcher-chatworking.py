...
class neo():
    def __init__ (self,environment,STMsize = 10 ):
        self.ltm = LTM()
        self.stm = STM()
    ...
    def searcher (self):
        ...
        relatedenvironments = self.ltm.get(ACPtrace_text, namespace = "environments")
        ...
        messages = self.SEARCHERPROMPT.format(relatedenvironments = str(relatedenvironments),
                        beliefenvironment = currentenvironment['env'],
    ...
    def actplancritique(self, actionplan):
        currentenvironmentaxioms = self.stm.get("currentenv")['env']["beliefaxioms"]
        messages = self.ACTPLANCRITIQUEPROMPT.format(beliefaxioms = currentenvironmentaxioms, actplan = actionplan)
    ...
    def actplan(self):
        ...
        relatedactionset = self.ltm.get(currentenvironment["description"], namespace = "actions",k = MAXRELATEDACTIONSET)
        relatedactionlist = []
        ...
        ###### try until correct plan is generated
            ...
            if False in isvalidids:
                errorfeedback = "Here is the last actionplan generated. "+ str(output)+ "\n But the requirements have invalid moduleids. Modify the actionplan to put correct moduleids from stored function modules"
                continue      
            ############ check if actionplan satisfies all constraints
            isvalid,reason = self.actplancritique(str(output))            
            if not isvalid:
                errorfeedback = "Here is the last actionplan generated. "+ str(output)+ "\n But it might be invalid due to "+reason
                continue
            break
        return output
    ...
    def actor(self,actionplan,currentcode="", codeerror = ""):
        ...
        envobjective = self.stm.get("currentenv")['env']["objective"]
        ACPtrace = self.stm.get("ACPtrace")
        prevactionplan = ACPtrace[-1]["actionplan"]["actionplan"] if ACPtrace else ""
        messages = self.ACTORPROMPT.format(envobjective = envobjective, \
                        actions = relatedactiontext, \
                        actionplan = actionplantext, \
                        prevactionplan = prevactionplan, \
    ...
    def actionplantoaction(self,output):
        ...
        while self.env.codeerror:
            ...
            while newaction is None:
                try:
                    newaction = self.actor(output,str(action),self.env.perception+"\n"+hint)
                except Exception as e:
                    pass
                    print ("Error", str(e))
    ...
    def run (self, lifetime = float("Inf")):   
        ...
        ########### store actionplan in stm #####################################################################
            ...
            newACPT = {"actionplan": output,"perception": str(self.env.perception)}
...
class neoenv():
    def __init__(self,
                 bootstrapbeliefenvironment = {"description": "","objective": "", "beliefaxioms":""},
    ...
    def getreward(self):
        self.perception = self.problemenv.getfeedback()
